        ...
        >
          <div className="relative flex justify-center pt-5">
            <motion.span
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.42, delay: Math.min(index * 0.07, 0.22) }}
            >
              <span className="h-1.5 w-1.5 rounded-full bg-neonCyan" />
            </motion.span>
            {index !== items.length - 1 ? (
              <span aria-hidden="true" className="absolute left-1/2 top-9 w-px -translate-x-1/2 overflow-hidden" style={{ height: "calc(100% + 1.25rem)" }}>
                <span className="absolute inset-0 bg-border/80" />
                <motion.span
                  className="absolute left-0 w-px bg-gradient-to-b from-neonCyan via-neonAmber to-neonCyan"
                  animate={
                    direction === "down" || !isPartial
                      ? { top: 0, bottom: "auto", height: `${segmentFill * 100}%` }
                      : { top: "auto", bottom: 0, height: `${segmentFill * 100}%` }
                  }
          ...
          <Card className="transition-colors hover:border-neonCyan/40">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="font-pixel text-xs text-neonAmber">{item.period}</span>
            </div>
