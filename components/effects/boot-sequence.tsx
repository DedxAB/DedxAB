      ...
      >
        <div className="w-[min(92vw,760px)] rounded-xl border border-neonCyan/35 bg-card/90 p-5 shadow-neon">
          <div className="mb-3 flex items-center justify-between border-b border-border/70 pb-2">
            <p className="font-pixel text-sm text-neonCyan">portfolio-shell installer v2.6.0</p>
            <p className="font-pixel text-xs text-muted-foreground">{Math.min(100, Math.round((index / bootLogs.length) * 100))}%</p>
          </div>
          <div ref={logRef} className="terminal-scrollbar h-64 overflow-y-auto rounded border border-border bg-muted/30 p-3 font-pixel text-[15px] leading-6">
            {bootLogs.slice(0, index).map((line, i) => (
              <p key={`${line.text}-${i}`} className={toneClass(line.tone)}>
                {line.text}
              </p>
            ))}
            {index < bootLogs.length && <p className="animate-pulse text-neonCyan">installing{dots}</p>}
          </div>
        </div>
      </motion.div>
    </AnimatePresence>
