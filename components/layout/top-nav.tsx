...
export function TopNav(): React.JSX.Element {
  ...
  const quickSocials = useMemo(() => portfolioConfig.socialLinks.slice(0, 4), []);
    ...
    <header className="sticky top-0 z-50 border-b border-border/70 bg-background/80 backdrop-blur-lg">
      <div className="px-4 md:px-6">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between py-2.5 md:py-2">
          <a href="#home" className="font-pixel text-neonCyan" onClick={playClick}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {theme === "dark" ? <Sun className="h-4 w-4 text-neonAmber" /> : <Moon className="h-4 w-4 text-neonCyan" />}
              </button>
            </div>
            <div className="flex items-center gap-2">
              {enabled ? <Volume2 className="h-4 w-4 text-neonGreen" /> : <VolumeX className="h-4 w-4 text-muted-foreground" />}
              <Switch checked={enabled} onCheckedChange={setEnabled} label="Sound and music" />
            </div>
          </div>
        </div>
      </div>
