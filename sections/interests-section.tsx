...
export function InterestsSection(): React.JSX.Element {
  const pickIcon = (interest: string): LucideIcon => {
    const match = iconByKeyword.find((item) => interest.toLowerCase().includes(item.keyword));
    return match?.icon ?? Sparkles;
  };

  return (
    <SectionShell id="interests" title="Interests & Hobbies" subtitle="Creativity and curiosity outside coding that influence how I design and build.">
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <p className="font-display text-sm leading-relaxed">{interest}</p>
              </div>
            </Card>
          );
        })}
